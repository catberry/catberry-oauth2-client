...
class RefreshTokenFlowEndpoint extends GrantFlowMiddlewareBase {
	...
	handler(request, response) {
		...
		this._eventBus.emit('trace', 'Obtaining access token for refresh token...');
		...
		this._obtainAuthorization(request)
			...
			.catch(reason => {
				...
				removeUri.query = removeUri.createQuery('');
	...
	_obtainAuthorization(request) {
		...
		return this._sender.send(request, refreshToken, this._scope);
	}
}
